...
Havven needs a direct incentive mechanism that can correct changes in the global collaterisation ratio, $C$, when the price of havvens or nomins changes. \\

\noindent Some of the equations below are defined in a discrete time domain and are referenced with a subscript $t$, which denotes the period at which they are defined.
...
\noindent Even so, though it might be unlikely, it's a simple matter to implement a democratic remedy, weighted by havven balance,
