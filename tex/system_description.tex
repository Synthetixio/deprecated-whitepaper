...
the nomin price, can be measured with an oracle. Initially, while nomin demand is low, we will use a seven day rolling average of the market price for both havvens and nomins. This rolling average is designed to smooth out fluctuations in the market price and increase the cost of attacking the system. However, once nomin transaction volume has increased to sufficient levels, we may consider an approach that allows incentives to be applied directly as a consequence of changes in nomin demand. In this way, the system can avoid the influence of speculative components of the havven price. \\

\noindent \textbf{Approximating Nomin Demand} \\

\noindent It is possible to approximate nomin demand internally as function of the transaction fees that the system charges. This computation is not vulnerable to instantaneous volume spikes, while taking the most recent transaction volumes to be the most highly-correlated with future volumes:

\begin{equation}
    D_{t} = \sum_{t'=1}^{t} \frac{F_{t - t'}}{(1 + r)^{t'}} \label{eq:price}
\end{equation}

where

\begin{align*} 
& D_{t} \text{ is the approximate nomin demand in period } t  \\
& F_t \text{ is the total fees collected in period } t\\
& r \text{ is a falloff term}  \\
\end{align*}

\noindent This can be computed efficiently, because $U_{h,t+1} = \frac{U_{h,t} + F_t}{r}$. 
Further, if it is assumed that the average fee take is approximated by $F_t$, and $t$ is large, then:

\begin{equation}
    U_{h,t} \approx \frac{1}{H} \sum_{t'=1}^{\infty} \frac{F_t}{(1 + r)^{t'}} = \frac{F_t}{H \cdot r}
\end{equation}
