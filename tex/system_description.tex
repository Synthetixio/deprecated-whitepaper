...
\noindent The fee rate paid to a \HAV{} holder that has escrowed their \HAV{} is $\alpha_r$. This rate changes with respect the individual's unique $U_i$. It increases linearly to a maximum at the optimal utilisation ratio $U_{opt}$, before quickly diminishing as $U_i$ approaches $U_{max}$. Beyond the maximum utilisation ratio $\alpha_r$ becomes 0. Note, $\alpha_r$ is applied to the pool of collected fees which is determined by $\alpha_c$. \\
...
\noindent The optimal utilisation ratio $U_{opt}$ is a target for \HAV{} holders to track against in order to optimise the amount of fees they receive. $U_{opt}$ is defined in terms of $P_n$ such that \HAV{} holders can influence the price of \NOM{} through directly controlling the supply of \NOM{} (a havven holder can change their individual utilisation ratio by buying or issuing more nomins). \\
