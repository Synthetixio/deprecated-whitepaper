...
\noindent This can be computed efficiently, because \(V_{t+1} = \frac{V_t + F_t \cdot P_{n,t}}{r}\). 
Further, if it is assumed that the average fee take is approximated by \(F_t \cdot P_{n,t}\), and \(t\) is large, then:

\vspace{2mm}

\begin{equation} \label{eq:havvenvalue}
    V_t \ \approx \ \sum_{t'=1}^{\infty} \frac{F_t \cdot P_{n,t}}{(1 + r)^{t'}} \ = \ \frac{F_t \cdot P_{n,t}}{H \cdot r}
\end{equation}
